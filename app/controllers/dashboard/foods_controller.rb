class Dashboard::FoodsController < DashboardController
  before_filter :food_find_by_id_and_establishment_id, only: [:edit, :update]
  ...
  def destroy
    ...
    redirect_to dashboard_establishment_foods_path, notice: t('.deleted')
  end
  ...
  def food_find_by_id_and_establishment_id
    ...
    return redirect_to dashboard_establishment_path(id: params[:establishment_id]), notice: t('.not_found') unless @food
  end
